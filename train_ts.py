...
from modules.model_ts import *
from modules.parser_ts import BiaffineParser
...
if __name__ == '__main__':
    ...
    tbert = BertEmbedding(data_path['pretrained']['bert_model'], nb_layers=args.bert_layer, merge='linear', use_proj=False, proj_dim=args.d_model)
    tmodel = BaseModel(args)
    tea_model = ParserModel(tmodel, tbert)
    
    sbert = BertEmbedding(data_path['pretrained']['bert_model'], nb_layers=args.bert_layer, merge='linear', use_proj=False, proj_dim=args.d_model)
    smodel = BaseModel(args)
    stu_model = ParserModel(smodel, sbert)
    ...
    tea_model = tea_model.to(args.device)
    stu_model = stu_model.to(args.device)
    biff_parser = BiaffineParser(tea_model, stu_model)
    biff_parser.summary()
    biff_parser.train(src_train_data, tgt_train_data, dev_data, test_data, args, dep_vocab)
    biff_parser.tea_stu_train(src_train_data, tgt_train_data, dev_data, test_data, args, dep_vocab)
