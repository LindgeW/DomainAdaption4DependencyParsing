...
class BiaffineParser(object):
    ...
    def train(self, src_train_data, tgt_train_data, dev_data, test_data, args, vocab):
        ...
        optimizer_bert_parameters = [
            {'params': [p for n, p in self.parser_model.bert_named_params()
        ...
        for ep in range(1, 1+args.epoch):
            ...
            for i, batch_data in enumerate(batch_iter(src_train_data, args.batch_size, shuffle=True)):
                ...
                if ep > 0:
                    ...
                    #gamma = min(1, 2 / (1+np.exp(-10 * (step / max_step))) - 1)
                    gamma = 2 / (1 + np.exp(-10 * ep / args.epoch)) - 1
                    #step += 1
                    print(gamma) 
                    arc_score, rel_score, dist_loss = self.parser_model(src_batcher[0], tgt_batcher[0], gamma)
                    #arc_score, rel_score, dist_loss, dom_loss = self.parser_model(src_batcher[0], tgt_batcher[0], gamma)
                    loss = self.calc_dep_loss(arc_score, rel_score, true_heads, true_rels, bert_lens.gt(0))
                    loss += gamma * dist_loss 
                    #loss += (0.01 * dom_loss + gamma * dist_loss)
                else:
                    arc_score, rel_score = self.parser_model(src_batcher[0])
                    loss = self.calc_dep_loss(arc_score, rel_score, true_heads, true_rels, bert_lens.gt(0))
